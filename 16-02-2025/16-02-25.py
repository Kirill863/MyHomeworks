...
class TranscriptionIterator:
    def __init__(stlf, transcription_data: List[Dict[str, list[Optional[float]]]]):
        self.transciption_data = transcription_data
        self.index = 0

    def __iter__(self)-> Iterator[TranscriptionChunk]:
        return self
    def _chunk_serialize(self, data: [Dict[str, List[Optional]]])
